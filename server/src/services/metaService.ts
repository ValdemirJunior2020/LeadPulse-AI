...
type PublisherPlatform = 'facebook' | 'instagram';
...
type PublishInput = {
  ...
  metaPublisherPlatforms: PublisherPlatform[];
};
...
function normalizePlatforms(platforms: PublisherPlatform[]): PublisherPlatform[] {
  const normalized = Array.from(new Set(platforms)).filter((platform): platform is PublisherPlatform =>
...
export async function listInstagramAccounts(): Promise<Array<{ id: string; username: string; pageId: string }>> {
  if (env.META_MOCK_MODE) {
    return [
      { id: 'ig_mock_1001', username: 'leadpulse_demo', pageId: 'page_mock_1001' },
      { id: 'ig_mock_1002', username: 'local_services_demo', pageId: 'page_mock_1002' },
    ];
  }

  throw new Error('Live Instagram account listing requires connected Facebook Pages and approved Meta permissions.');
}

export async function publishPausedCampaign(input: PublishInput): Promise<MetaPublishResponse> {
  const platforms = normalizePlatforms(input.metaPublisherPlatforms);
  ...
  await setDoc('ads', adId, {
    ...
    creativeIdentity: {
      ...
      instagram_actor_id: platforms.includes('instagram') ? input.instagramActorId : undefined,
    },
    createdAt,
    updatedAt: createdAt,
  });
